...
- Can a model trained solely with synthetic data beat the performance of a model trained with real data?
...
1. Exclusively with synthetic data constrained to 500.000 images.
2. Exclusively with synthetic data with no limit on the number of images.
3. Combining real and synthetic data.

### Datasets

One of the novelties in this second edition of the FRCSyn Challenge is that there are no fixed datasets for the training with synthetic data. **We allow participants to use any method to create the synthetic datasets for the three sub-tasks of the challenge. However, the real dataset for sub-task 2 will be fixed to the [CASIA-WebFace](https://arxiv.org/pdf/1411.7923.pdf) dataset.** We list some state-of-the-art methods that could be used as an entry point:
...
**CASIA-WebFace**: The dataset that provides the training data from real images for this challenge. A dataset containing 494,414 face images of 10,575 real identities collected from the web.
...
1. Fill up [this form](https://docs.google.com/forms/d/e/1FAIpQLScpQ3IqHMojyNClNADVKilVZF1oidjLMV-Q2MaGrn7m6enT9A/viewform?vc=0&c=0&w=1&flr=0) including your information.
...
To determine the winners of sub-tasks 1.1, 1.2, and 1.3 we consider Trade-off Accuracy, defined as the difference between the average and standard deviation of accuracy across demographic groups. To determine the winners of sub-tasks 2.1, 2.2, and 2.3 we consider the average of verification accuracy across datasets.
